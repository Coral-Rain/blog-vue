        ...
        <div class="six wide computer six wide tablet sixteen wide mobile column right aligned bottom aligned ta-left-mobile">
          <div class="ui inline dropdown" tabindex="0">
            ...
            <div class="menu" tabindex="-1">
              <router-link class="item" :class="!this.$route.query.sort || this.$route.query.sort === 'follow_time' ? 'selected active' : ''" :to="{query: {sort: 'follow_time'}}">关注时间</router-link>
              <router-link class="item" :class="this.$route.query.sort === 'recent_login' ? 'selected active' : ''" :to="{query: {sort: 'recent_login'}}">最近登录</router-link>
            </div>
          </div>
        </div>
        ...
        <div class="item" v-for="l in list">
          ...
          <div class="ui image user-actions" v-if="tergetUserId !== l.follower">
            <div class="ui basic mini buttons">
              ...
              <a class="ui button follow-btn" @click="cancleFollow(l)" v-if="l.follow" @mouseenter="overFollowed($event)" @mouseleave="leaveFollowed($event)">
