...
(function ($) {
    ...
    $.supersized = function (b) {
        ...
        d.init = function () {
            ...
            d.options = $.supersized.vars.options;
            d._build()
        };
        d._build = function () {
            var g = 0, e = "", j = "", h, f = "", i;
            ...
            if (d.options.slide_links) {
                $(vars.slide_list).html(j)
            }
            if (d.options.thumb_links && vars.thumb_tray.length) {
                $(vars.thumb_tray).append('<ul id="' + vars.thumb_list.replace("#", "") + '">' + f + "</ul>")
            }
            $(d.el).append(e);
            if (d.options.thumbnail_navigation) {
                ...
                $(vars.prev_thumb).show().html($("<img/>").attr("src", d.options.slides[prevThumb].image));
                vars.current_slide == d.options.slides.length - 1 ? nextThumb = 0 : nextThumb = vars.current_slide + 1;
                $(vars.next_thumb).show().html($("<img/>").attr("src", d.options.slides[nextThumb].image))
            }
            d._start()
        };
        d._start = function () {
            ...
            if (d.options.slides.length > 1) {
                if (d.options.slides.length > 2) {
                    ...
                    var q = $('<img src="' + d.options.slides[loadPrev].image + '"/>');
            ...
            var l = $('<img src="' + api.getField("image") + '"/>');
            ...
            if (d.options.slides.length > 1) {
                ...
                var e = $('<img src="' + d.options.slides[loadNext].image + '"/>');
        ...
        d.resizeNow = function () {
            return d.$el.each(function () {
                ...
                if (d.options.image_protect) {
                    $("img", d.el).bind("contextmenu mousedown", function () {
                        return false
                    })
                }
                return false
        ...
        d.nextSlide = function () {
            ...
            var g = $(d.el + " li:eq(" + vars.current_slide + ")"), i = d.$el.find(".prevslide");
            ...
            if (!$(k).html()) {
                ...
                var f = $('<img src="' + d.options.slides[loadSlide].image + '"/>');
            ...
            if (d.options.thumbnail_navigation == 1) {
                vars.current_slide - 1 < 0 ? prevThumb = d.options.slides.length - 1 : prevThumb = vars.current_slide - 1;
                $(vars.prev_thumb).html($("<img/>").attr("src", d.options.slides[prevThumb].image));
                nextThumb = loadSlide;
                $(vars.next_thumb).html($("<img/>").attr("src", d.options.slides[nextThumb].image))
            }
            if (typeof theme != "undefined" && typeof theme.beforeAnimation == "function") {
                theme.beforeAnimation("next")
            }
        ...
        d.goTo = function (f) {
            ...
            if (d.options.slide_links) {
                ...
                $(vars.slide_list + "> li").eq((e - f)).addClass("current-slide")
            }
            if (d.options.thumb_links) {
                $(vars.thumb_list + "> .current-thumb").removeClass("current-thumb");
                $(vars.thumb_list + "> li").eq((e - f)).addClass("current-thumb")
            }
        };
        ...
        d.afterAnimation = function () {
            ...
            if (vars.update_images) {
                ...
                $(".prevslide").removeClass("prevslide");
                $(d.el + " li:eq(" + setPrev + ")").addClass("prevslide")
            }
            vars.in_animation = false;
